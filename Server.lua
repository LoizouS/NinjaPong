...
local function connectClients() --checks to see if any new clients wish to connect
    ...
    while(message) do
        ...
        print("adding clients",message[1], message["1"],clientPort )
        if(message) then
            if(message[1] and message[1]=="CoronaMultiplayer" and message[2]==applicationName) then --this is the protocol id	
                ...
                if(availibleClients[clientIP]==nil or availibleClients[clientIP].clientPort~=clientPort) then --the client ID and port must be the same
                    
                    
                    
                    if(availibleClients[clientIP] and availibleClients[clientIP].clientPort~=clientPort) then 
                        --we must disconnect the previous connection that was here
                        for i = 1, numClients do
                            if(clientsInfo[i] and clientsInfo[i].ip == clientIP) then                           
                                UDPClients[i]:close()
                                UDPClients[i] = nil
                        ...
                        numClients = numClients+1 --we are dealing with a new connection
                    end       
                    ...
                    availibleClients[clientIP] = {name = message[3], port = port, timeoutTime = timeoutTime, ip = clientIP, clientPort = clientPort}
                    ...
                    for i = 1, numClients do
                        if(  clientsInfo[i] and clientsInfo[i].ip == clientIP) then                           
                            ...
                            handshakeTable[5] = i
                            UDPClient:sendto(json.encode(handshakeTable),clientIP,clientPort)						
                        end
                    end
                end
            end
